...
if (pool) {
  const exsisting = await pool.query("SELECT 1 FROM Users WHERE USErname=$1" , 
    
    [username]
  );

  if (exsistingUserResult.rowCount > 0) {
    return res
    .status(409)
    .json({ ok: false, error: "user alredy exsists" });
  }
...
if (usersMemory.has(username)) {
  return res
  .status(409)
  .json({ ok: false, error: "username alredy exsists" });
}
...
usersMemory.set(username, newUser);
 ...
 return res.status(201).json({
  ...
  user: {
    ...
    tosVersion: newUser.tosVersion,
    consentedAt: newUser.consentedAt,
    createdAt: newUser.createdtedAt,
  },
});
});
...
//--------------------------------her-----------------------------------------//
  ...
  if (!username || !password) {
    return res.
    status(400)
    .json({ ok: false, error: "username amd passowrd are rquierd" });
  ...
  if (pool) {
    const userLookupResult  = await pool.query(
      "SELECT password_hash, passoword_salt FROM users Where username = $1" ,
      [username]
    );
    ...
    const dbUser = userLookupResult.row[0];
...
//-------------db path--------------------------------------------------------//
  ...
  if (pool) {
   ...
   if (userLookupResult.rowCount === 0) {
    ...
    const uptateResult = await pool.query(
      `UPTATE users 
      SET tos_version = $2, consented_at = NOW()
       WHERE username = $1
       RETURNING id, username, tos_version, consented_at, created_at`,
    ...
    const updatedUserRow = uptateResult.row[0];
  ...
  MemoryUser.tosVersion = tosVersion;
  MemoryUser.consentedAt = new Date().toISOString();
  MemoryUser.set(username, MemoryUser);
